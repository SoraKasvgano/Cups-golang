...
func TestHandleGetPrinterSupportedValuesClassAddressedViaPrintersPath(t *testing.T) {
	s := newMoveTestServer(t)
	ctx := context.Background()
	...
	req.Operation.Add(goipp.MakeAttribute("attributes-charset", goipp.TagCharset, goipp.String("utf-8")))
	req.Operation.Add(goipp.MakeAttribute("attributes-natural-language", goipp.TagLanguage, goipp.String("en-US")))
	req.Operation.Add(goipp.MakeAttribute("printer-uri", goipp.TagURI, goipp.String("ipp://localhost/printers/Team")))
	...
	for _, name := range []string{
		...
		if !hasSupportedValueAttr(resp.Printer, name) {
			t.Fatalf("missing supported value attr %q in class-via-printers response", name)
		}
	}
}
